export const playerTools = [
    {
        name: "get_players",
        description: "Get all available music players. Use this to find player IDs for playback control.",
        inputSchema: {
            type: "object",
            properties: {
                include_unavailable: {
...
function formatPlayer(player) {
    ...
    const volume = player.volume_level !== null ? `${player.volume_level}%` : "N/A";
    const powered = player.powered !== null ? (player.powered ? "On" : "Off") : "N/A";
...
export async function handlePlayerTool(client, name, args) {
    switch (name) {
        case "get_players": {
            const includeUnavailable = args?.include_unavailable || false;
            const includeDisabled = args?.include_disabled || false;
            const players = await client.executeCommand("players/all", {
        ...
        case "get_player": {
            ...
            const player = await client.executeCommand("players/get", {
                player_id: playerId,
            });
            if (!player) {
        ...
        case "get_player_by_name": {
            ...
            const player = await client.executeCommand("players/get_by_name", {
                name: playerName,
            });
            if (!player) {
                return {
        ...
        case "get_all_queues": {
            const queues = await client.executeCommand("player_queues/all");
        ...
        case "create_player_group": {
            const targetPlayer = args?.target_player;
            const memberPlayerIds = args?.member_player_ids;
