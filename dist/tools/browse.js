export const browseTools = [
    {
        name: "browse_library",
        description: "Browse the music library. Can browse root, specific paths, or provider content. Use this to explore available music.",
        inputSchema: {
            type: "object",
            properties: {
                path: {
                    type: "string",
                    description: "Optional path to browse. Leave empty for root. Use provider URIs to browse specific providers.",
                },
            },
            required: [],
        },
    },
    {
        ...
        inputSchema: {
            ...
            properties: {
                ...
                order_by: {
                    type: "string",
                    description: "Sort order: 'name', 'sort_name', 'timestamp_added', 'timestamp_modified', 'last_played', 'play_count', 'random' (default: 'sort_name')",
                },
            },
            required: ["media_type"],
        },
        ...
        inputSchema: {
            ...
            properties: {
                ...
                media_types: {
                    type: "array",
                    items: { type: "string" },
                    description: "Filter by media types (e.g., ['track', 'album']). Default: all types",
                },
            },
            required: [],
        },
...
export async function handleBrowseTool(client, name, args) {
    switch (name) {
        case "browse_library": {
            const path = args?.path;
            const results = await client.executeCommand("music/browse", { path });
        ...
        case "get_library_items": {
            ...
            const favorite = args?.favorites_only;
            const orderBy = args?.order_by || "sort_name";
            if (!mediaType) {
                return {
            ...
            const results = await client.executeCommand(endpoint, {
                limit,
                offset,
                search,
                favorite,
        ...
        case "get_item_children": {
            const uri = args?.uri;
            const childType = args?.child_type;
            if (!uri) {
                return {
            ...
            if (mediaType === "album" && childType === "tracks") {
                const album = await client.executeCommand("music/albums/get", {
                    item_id: itemId,
                    provider_instance_id_or_domain: provider,
                });
                const results = await client.executeCommand("music/albums/tracks", {
                    item_id: itemId,
                    provider_instance_id_or_domain: provider,
                });
                if (!results || results.length === 0) {
            ...
            if (mediaType === "playlist" && childType === "tracks") {
                ...
                const offset = args?.offset || 0;
                const playlist = await client.executeCommand("music/playlists/get", {
                    item_id: itemId,
                    provider_instance_id_or_domain: provider,
                });
                const results = await client.executeCommand("music/playlists/tracks", {
                    item_id: itemId,
                    provider_instance_id_or_domain: provider,
                    limit,
                    offset,
            ...
            if (mediaType === "artist" && childType === "albums") {
                const artist = await client.executeCommand("music/artists/get", {
                    item_id: itemId,
                    provider_instance_id_or_domain: provider,
                });
                const results = await client.executeCommand("music/artists/albums", {
                    item_id: itemId,
                    provider_instance_id_or_domain: provider,
                });
                if (!results || results.length === 0) {
            ...
            if (mediaType === "artist" && childType === "tracks") {
                ...
                const results = await client.executeCommand("music/artists/tracks", {
                    item_id: itemId,
                    provider_instance_id_or_domain: provider,
                });
                if (!results || results.length === 0) {
        ...
        case "get_recommendations": {
            const results = await client.executeCommand("music/recommendations");
            if (!results || results.length === 0) {
                return {
        ...
        case "get_recently_played": {
            ...
            const mediaTypes = args?.media_types;
            const results = await client.executeCommand("music/recently_played_items", {
                limit,
        ...
        case "get_item_details": {
            ...
            const result = await client.executeCommand("music/item_by_uri", {
                uri,
            });
            if (!result) {
