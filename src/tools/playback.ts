...
import { MusicAssistantClient, PlayerQueue, QueueItem } from "../client.js";

export const playbackTools: Tool[] = [
  {
    name: "play_media",
    description:
      "Play media (track, album, playlist, artist, radio, etc.) on a player. Use this to start playing something.",
    inputSchema: {
      type: "object",
      properties: {
        player_id: {
          type: "string",
          description:
            "The ID of the player to play on. Use get_players to find available players.",
        },
        media_uri: {
          type: "string",
          description:
            "The URI of the media to play (e.g., 'spotify://track/abc123'). Can be a single item or multiple URIs as array.",
        },
        media_uris: {
          type: "array",
          items: { type: "string" },
          description:
            "Multiple URIs to play (for adding multiple items at once)",
        },
        queue_option: {
          type: "string",
          enum: ["play", "replace", "next", "replace_next", "add"],
          description:
            "How to handle the queue: 'play' (play now), 'replace' (replace queue and play), 'next' (play next), 'replace_next' (replace upcoming), 'add' (add to end). Default: 'play'",
        },
        radio_mode: {
          type: "boolean",
          description:
            "Enable radio mode (auto-add similar tracks). Default: false",
        },
      },
      required: ["player_id"],
    },
    ...
    inputSchema: {
      ...
      properties: {
        ...
        item_id_or_index: {
          type: "string",
          description:
            "Queue item ID or index to remove",
        },
      },
    ...
    inputSchema: {
      ...
      properties: {
        ...
        position_shift: {
          type: "number",
          description:
            "Number of positions to shift (negative = earlier, positive = later)",
        },
      },
...
export async function handlePlaybackTool(
  ...
  switch (name) {
    case "play_media": {
      const mediaUri = args?.media_uri as string | undefined;
      const mediaUris = args?.media_uris as string[] | undefined;
      const queueOption = (args?.queue_option as string) || "play";
      const radioMode = (args?.radio_mode as boolean) || false;
    ...
    case "set_volume": {
      const volumeLevel = args?.volume_level as number;
    ...
    case "set_shuffle": {
      const shuffle = args?.shuffle as boolean;
    ...
    case "set_repeat": {
      const repeatMode = args?.repeat_mode as string;
      if (!repeatMode) {
        return {
    ...
    case "get_queue": {
      ...
      const queue = await client.executeCommand<PlayerQueue>(
        "player_queues/get",
        { queue_id: playerId }
      );

      const items = await client.executeCommand<QueueItem[]>(
        "player_queues/items",
        {
          queue_id: playerId,
          limit,
    ...
    case "remove_queue_item": {
      const itemIdOrIndex = args?.item_id_or_index as string;
      if (!itemIdOrIndex) {
        return {
          content: [
    ...
    case "move_queue_item": {
      const queueItemId = args?.queue_item_id as string;
      const positionShift = args?.position_shift as number;
    ...
    case "transfer_queue": {
      const sourcePlayerId = args?.source_player_id as string;
      const targetPlayerId = args?.target_player_id as string;
    ...
    case "power": {
      const powered = args?.powered as boolean;
