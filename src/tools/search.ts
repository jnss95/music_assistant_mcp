...
import { MusicAssistantClient, SearchResults, MediaType } from "../client.js";

// Media types for search filtering
const SEARCHABLE_MEDIA_TYPES: MediaType[] = [
  "artist",
  "album",
  "track",
  "playlist",
...
export const searchTools: Tool[] = [
  {
    name: "search",
    description:
      "Search for music in the library. Can search across all media types or filter by a specific type (track, album, artist, playlist, radio, podcast, audiobook).",
    inputSchema: {
      type: "object",
      properties: {
        query: {
        ...
        media_type: {
          ...
          description:
            "Optional: Filter results to a specific media type. If not provided, searches all types.",
        },
        limit: {
          type: "number",
          description:
            "Maximum number of results (default: 25, or 10 per type when searching all types)",
        },
        library_only: {
          type: "boolean",
          description:
            "Only search items in the library (default: false, searches all providers)",
        },
      },
      required: ["query"],
    },
          ...
          .map((t) => {
            const artists =
              "artists" in t
                ...
                : "";
            const album =
              "album" in t && t.album ? ` (${t.album.name})` : "";
            const duration =
    ...
    sections.push(
      "## Podcasts\n" +
        results.podcasts
          .map((p) => {
            const publisher =
              "publisher" in p && p.publisher ? ` (${p.publisher})` : "";
...
export async function handleSearchTool(
  ...
  const mediaType = args?.media_type as MediaType | undefined;
  const libraryOnly = (args?.library_only as boolean) || false;
  ...
  const mediaTypes: MediaType[] = mediaType ? [mediaType] : SEARCHABLE_MEDIA_TYPES;
  const limit = (args?.limit as number) || (mediaType ? 25 : 10);

  const results = await client.executeCommand<SearchResults>("music/search", {
    search_query: query,
